...
class MyAssistant:
    ...
    def __init__(self):
        ...
        self._led = Led(channel=17)
        self._button = Button(channel=27)
    ...
    def _process_event(self, event):
        ...
        if event.type == EventType.ON_START_FINISHED:
            self._led.status = Led.BEACON_DARK  # Ready.
            ...
            self._can_start_conversation = False
            self._led.state = Led.ON  # Listening.

        elif event.type == EventType.ON_END_OF_UTTERANCE:
            self._led.state = Led.PULSE_QUICK  # Thinking.

        elif (event.type == EventType.ON_CONVERSATION_TURN_FINISHED
              or event.type == EventType.ON_CONVERSATION_TURN_TIMEOUT
              or event.type == EventType.ON_NO_RESPONSE):
            self._led.state = Led.BEACON_DARK  # Ready.
            self._can_start_conversation = True
